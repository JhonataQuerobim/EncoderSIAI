/*
    CalculaPosVel.h - Classe que interpreta o contador que conta os pulsos do encoder do robô da disciplina.
    Autores: Carlos Gomes, Gustavo de Souza, Jhonata Querobim e Vinicíus Peixoto
    Domínio público
*/

#ifndef CalculaPosVel_h_
#define CalculaPosVel_h_

class CalculaPosVel
{
    public:
        CalculaPosVel(int *p, int sent);                 //Construtor
        CalculaPosVel(const CalculaPosVel &copia);            //Construtor de cópia
        ~CalculaPosVel();                                //Destrutor
        int getSentido();
        double getPosicao();
